...
def clean_ine_data(file_path, output_path, municipios_regions_path):
    ...
    try:
        ...
        # --- STEP 5: Extract correct studies headers from row 7 of the raw CSV. ---
        ...
        # The rest are studies columns.
        actual_columns = len(df.columns) - 4
        studies_headers = df_headers.iloc[0, 4:].dropna().tolist()
        
        if len(studies_headers) > actual_columns:
            studies_headers = studies_headers[:actual_columns]
        elif len(studies_headers) < actual_columns:
            studies_headers += ["Unknown_Column"] * (actual_columns - len(studies_headers))
        
        # --- STEP 6: Apply correct column names. ---
        new_columns = ["Place of Residence", "Age Group", "Gender", "Year"] + studies_headers
        ...
        # --- STEP 13: Ensure at least one studies column has a non-zero entry. ---
        # The studies columns are between "Gender" and "freguesia_code"/"municipio"/"region".
        # Indexes: 0=Place,1=AgeGroup,2=Gender -> studies start at col index 3 until the last 3 columns
        studies_cols = df.columns[3:-3]  # everything except the last 3 columns (freguesia_code, municipio, region)
        df = df[(df[studies_cols] != '0').any(axis=1)]  # must have at least one non-zero in studies

        # --- STEP 14: Convert those studies columns to numeric & fill NaNs with 0. ---
...
if __name__ == "__main__":
    ...
    data_raw_dir = os.path.join(base_dir, "data_raw_studies")  # Changed from data_raw
    data_cleaned_dir = os.path.join(base_dir, "data_cleaned_studies")  # Changed from data_cleaned
    old_data_raw_dir = os.path.join(base_dir, "old_data_raw_studies")  # Changed from old_data_raw
